...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        header.addView(button("âš™ Settings") { showSettings() })
        root.addView(header)
    ...
    private fun showDashboard() {
        ...
        val serviceStatus = TextView(this).apply { text = "â— Siap offline"; textSize = 14f; setTextColor(if (darkMode) Color.WHITE else Color.BLACK); setPadding(0, 0, 0, 4) }
        ...
        val mic = button("ðŸŽ™") { startForegroundService(Intent(this, VoiceService::class.java)); serviceStatus.text = "â— Mendengarkan Volume Up"; serviceStatus.setTextColor(if (darkMode) Color.WHITE else Color.BLACK) }
    ...
    private fun showSettings() {
        ...
        layout.addView(LinearLayout(this).apply { gravity = Gravity.CENTER_VERTICAL; addView(button("â†") { showDashboard() }); addView(TextView(this@MainActivity).apply { text = "PENGATURAN"; textSize = 24f; typeface = pixelTypeface; setTextColor(if (darkMode) Color.WHITE else Color.BLACK); setPadding(16, 0, 0, 0) }) })
        layout.addView(TextView(this).apply { text = "KONFIGURASI OFFLINE â€¢ BITWHISPER"; typeface = pixelTypeface; textSize = 12f; setTextColor(if (darkMode) Color.LTGRAY else Color.DKGRAY); setPadding(0, 8, 0, 20) })
        val models = ModelManager(this); val wakeModel = WakeWordModelManager(this); val enrollment = WakeWordEnrollmentStore(this); val status = TextView(this).apply { text = "Whisper: ${if (models.isWhisperReady()) "siap" else "belum"}\\nChatbot: ${if (models.isChatReady()) "siap" else "belum"}\\nWake word: ${if (enrollment.sampleCount >= 3 || wakeModel.isReady()) "siap" else "belum"}" }
